...
A cost-state covering calls from more than one backend is stored as `mixed`. Its dollars and authoritative token counters remain a single whole-session total: they are never prorated across backends, models, messages, or agents. Consequently the session appears in **All** and **Mixed**, but not in a component backend view. Agent rows show transcript-observed calls for audit only; when a cost-state exists, their token totals need not sum to the authoritative session counters and no per-agent cost is invented.
